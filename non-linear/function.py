...
def cell_type_ref_train(data,types,num_for_mean,split_num,eachtype_num,shap_index):
    ...
    for i in range(len(types)):
        ...
        # np.random.shuffle(array_temp)
        data_matrix_list_train.append(array_temp[num_for_mean:split_num])
        data_matrix_list_val.append(array_temp[split_num:eachtype_num])
    ...
    return cell_type_ref,data_matrix_list_train,data_matrix_list_val
...
class YourModel(nn.Module):
    def __init__(self, input_size, hidden_units, do_rates, n_classes):
        super(YourModel, self).__init__()
        
        self.fc1 = nn.Linear(input_size, hidden_units[0])
        self.dropout1 = nn.Dropout(p=do_rates[0])
        
        self.fc2 = nn.Linear(hidden_units[0], hidden_units[1])
        self.dropout2 = nn.Dropout(p=do_rates[1])
        
        self.fc3 = nn.Linear(hidden_units[1], hidden_units[2])
        self.dropout3 = nn.Dropout(p=do_rates[2])
        
        self.fc4 = nn.Linear(hidden_units[2], hidden_units[3])
        self.dropout4 = nn.Dropout(p=do_rates[3])
        
        self.fc5 = nn.Linear(hidden_units[3], n_classes)
...
def batch_data(batchsize,dim,data_matrix_list):
    ...
    for i in range(batchsize):
        ratio_num,target= ratio_num_generate(50*2,[1,3,5],6)
        input = MixDataset(data_matrix_list,1,ratio_num)
        input = torch.from_numpy(input).float()
        input = norm_method('z_score',input)        
        batch_input[i] = input.numpy()
